...
- MADHYN: Autonomous AI development environment and engineering command center.
...
const MADHYN_KNOWLEDGE = `
# MADHYN Knowledge (Autonomous AI Development Environment)
MADHYN is an autonomous AI development environment by RexyCore. It functions as an intelligent software engineering environment rather than a conventional AI coding assistant.
Tagline: "Your command center for autonomous development."

CORE PHILOSOPHY:
- MADHYN is designed around the loop: CONTEXT → REASON → ACTION → PROOF (or expanded: Research → Understand → Plan → Build → Run → Test → Observe → Fix → Verify → Learn).
- Instead of just generating code, it understands the project, researches, plans, modifies files, runs terminal commands, tests, and verifies results.
- Human directs the objective (e.g. "Add authentication"). MADHYN executes the workflow. Human decides on high-impact/ambiguous choices.

FEATURES & CAPABILITIES:
- Project Understanding & Memory: It builds a living context of folder structures, architecture, dependencies, and decisions so it doesn't repeatedly rediscover them.
- Extensible Tool System: Interacts with the real world safely (secure file operations, terminal execution, web research, testing frameworks, plugins).
- Repository Awareness: Operates directly inside Git repositories, branches, and diffs.
- Testing & Iteration (Proof): It runs tests on its own changes. If a test fails, it observes the failure, diagnoses, adapts, and tries again.
- Security Model: All actions pass through a security/policy layer. High-impact actions (destructive file ops, network actions) require authorization.
- Model Freedom: The intelligence layer is interchangeable. You can use local models (for absolute privacy), heavy reasoning cloud models, or fast models.
- Observability: Activity transparency is key. The user sees what MADHYN is doing (e.g. "Scanning architecture...", "Running tests...", "All tests passed.") instead of internal model thoughts.
- Dual Interaction: Supports both conversational Q&A ("Why is this API returning 401?") and executable delegation ("Fix the auth flow.").

IMPORTANT RESTRICTIONS:
MADHYN is NOT just a VS Code clone, chatbot, code autocomplete window, or generic wrapper. It is an acting development command center.
`;
...
export function getRelevantKnowledge(userMessage) {
  ...
  const isAboutMadhyn = /(madhyn|autonomous|development environment|command center|context reason action proof)/.test(msg);
  ...
  if (isAboutMadhyn) context += MADHYN_KNOWLEDGE + "\n";
